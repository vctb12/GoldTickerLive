...
- Added **Source Sans 3** (Latin UI, tabular lining figures) + **Cairo** (Arabic) as subset woff2
  under `assets/fonts/`.
...
New tokens for consistent component styling: `--text-muted`, `--surface-raised`, `--price-up`,
`--price-down`, `--price-up-bg`, `--price-down-bg`, `--price-up-border`, `--price-down-border`,
`--font-feature-tabular`.

### 6. Price numerics (`base.css`)

Extended tabular lining rules to `.hlc-price`, `.cp-price-value`, `.order-price-badge`,
`[data-testid='gold-price']`.

## Why

The audit identified a **serif/sans split** and **Cairo-only Latin** as cheap-feeling. Financial
reference sites need:
...
- New `styles/partials/price-display.css` — shared primitive for reference prices, direction/change
  badges, freshness chips, retail vs reference distinction.
- **Home:** `price-hero` wrapper; freshness chip moved adjacent to price (bound to number);
  reference kind chip on spot title; footer simplified.
- **Country:** Hero prices use `--text-data-lg`; `freshness-chip` on update line; retail intel stat
  visually distinct.
- **Order:** Reference price badge restyled (ink on surface, not gold pill); pending breakdown
  guided empty state.

### Why

The price is the product — one authoritative treatment, direction shown with colour **and** glyph,
freshness legible beside the number, reference vs retail instantly distinguishable.
