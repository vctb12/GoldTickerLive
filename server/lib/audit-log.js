...
function exportToCSV() {
  ...
  // Properly quote and escape a CSV field to prevent CSV injection
  function csvField(value) {
    const str = value == null ? '' : String(value);
    // Prefix with a tab any cell that starts with =, +, -, or @ to neutralise
  ...
  return [headers.map(csvField), ...rows].map((row) => row.join(',')).join('\n');
}
