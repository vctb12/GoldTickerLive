...
- **Tracker** (`tracker.html` / `tracker-pro.js`): `tracker/state.js` module; state also serialised
  into the URL hash for shareability
- **Persistence**: `lib/cache.js` wraps `localStorage` for gold price, FX rates, history, and user
  preferences
...
- Arabic locale flips layout direction via RTL CSS; `config/countries.js` carries both EN and AR
  country names

### Adding a New Country

1. Add an entry to `config/countries.js` (code, names, currency, group, decimals, peg flag if
   applicable)
2. Create `countries/<slug>.html` — follow an existing country page as a template; it uses
   `country-page.js` / `country-page.css`
