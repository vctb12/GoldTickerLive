...
export function renderMiniStrip() {
  ...
  const spot = _currentSpot();
  if (!spot) { _el.miniStrip.textContent = 'Waiting for live data…'; return; }
  const selected = _priceFor({ currency: _state.selectedCurrency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot });
...
export function renderKaratTable() {
  ...
  const spot = _currentSpot();
  if (!spot) { _el.karatTable.innerHTML = '<tr><td colspan="4">Waiting for live data…</td></tr>'; return; }
  const price24 = _priceFor({ currency: _state.selectedCurrency, karat: '24', unit: _state.selectedUnit, spot });
  _el.karatTable.innerHTML = KARATS.map(k => {
    const p = _priceFor({ currency: _state.selectedCurrency, karat: k.code, unit: _state.selectedUnit, spot });
...
export function renderMarkets() {
  ...
  const spot = _currentSpot();
  if (!spot) { _el.marketBoard.innerHTML = '<p style="padding:1rem;color:var(--tp-text-muted)">Waiting for live data…</p>'; return; }
  ...
  if (sortValue === 'high') {
    filtered.sort((a, b) => (_priceFor({ currency: b.currency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot }) || 0) - (_priceFor({ currency: a.currency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot }) || 0));
  } else if (sortValue === 'low') {
    filtered.sort((a, b) => (_priceFor({ currency: a.currency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot }) || 0) - (_priceFor({ currency: b.currency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot }) || 0));
  ...
  _el.marketBoard.innerHTML = filtered.map(c => {
    const cur = c.currency;
    const p = _priceFor({ currency: cur, karat: _state.selectedKarat, unit: _state.selectedUnit, spot });
...
export function renderWatchlist() {
  ...
  if (!favs.length || !spot) {
    _el.watchlistGrid.innerHTML = '<p style="color:var(--tp-text-muted);font-size:0.85rem">No favorites set. Add currencies via the Compare tab. Your watchlist will appear here for quick reference.</p>';
    return;
  }
  _el.watchlistGrid.innerHTML = favs.map(cur => {
...
export function renderDecisionCues() {
  ...
  const spot = _currentSpot();
  if (!spot) { _el.decisionCues.innerHTML = ''; return; }
  const lines = [
  ...
  _el.decisionCues.innerHTML = lines.map(l => `<div class="tracker-note-item">${l}</div>`).join('');
}
...
export function renderPresets() {
  ...
  _el.presetList.innerHTML = presets.length
    ? presets.map((p, i) => {
        const isCurrent = _state.selectedCurrency === p.currency &&
...
export function renderPlanners() {
  ...
  if (_el.budgetResults) {
    ...
    const p = _priceFor({ currency: _state.selectedCurrency, karat: _state.selectedKarat, unit: 'gram', spot });
    _el.budgetResults.innerHTML = p && net
  ...
  if (_el.positionResults) {
    ...
    if (entry && qty && p) {
      ...
      _el.positionResults.innerHTML = '<p style="color:var(--tp-text-muted)">Enter entry price and quantity above.</p>';
    }
  }
  ...
  if (_el.jewelryResults) {
    ...
    const p = _priceFor({ currency: _state.selectedCurrency, karat: karatCode, unit: 'gram', spot });
    if (weight && p && karat) {
      ...
      _el.jewelryResults.innerHTML = '<p style="color:var(--tp-text-muted)">Enter weight and select karat above.</p>';
    }
  }
  ...
  if (_el.accumResults) {
    ...
    if (p && monthly && target) {
      ...
      _el.accumResults.innerHTML = '<p style="color:var(--tp-text-muted)">Enter monthly contribution and target quantity above.</p>';
    }
  }
}
...
export function renderArchive() {
  ...
  _el.archiveBody.innerHTML = rows.map(r => {
    const aed24 = _priceFor({ currency: 'AED', karat: '24', unit: 'gram', spot: r.spot });
    const selected = _priceFor({ currency: _state.selectedCurrency, karat: _state.selectedKarat, unit: _state.selectedUnit, spot: r.spot });
    return `<tr>
  ...
  if (_el.archiveMeta) {
    const sourceInfo = _state.history.some(r => r.source === 'live' || r.source === 'session-cache')
      ? 'session + baseline'
      : 'baseline';
...
export function renderBrief() {
  ...
  if (!spot) {
    ...
    _el.briefCopy.textContent = 'Gold price data is loading. If this persists, the API may be temporarily unavailable — last cached price will be shown when available.';
    return;
  }
  const aed24 = _priceFor({ currency: 'AED', karat: '24', unit: 'gram', spot });
  ...
  _el.briefCopy.textContent = `UAE 24K: AED ${aed24 ? aed24.toFixed(2) : '—'}/g. `
