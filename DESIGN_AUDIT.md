...
**VERIFIED (screenshots):** EN desktop homepage hero, country UAE, city Dubai gold-rate, order-gold,
chart — see `docs/audit-screenshots/`.  
**UNVERIFIED / ASSUMED:** AR RTL layout quality — automated captures did not reliably confirm
`dir="rtl"` / translated headings on all city templates; manual RTL walk still required in Area E.

---

## Scoring key

Each dimension scored **1–5** (1 = hurts trust/feel, 5 = premium). Scores reflect **current
`main`**, not planned fixes.
...
**Files:** `countries/uae/dubai/gold-rate/index.html` (representative of 100+ generated city
gold-rate pages), `scripts/node/consolidate-country-pages.js`

> Note: City **hub** stubs (`/countries/uae/dubai/`) use `countries/stub-city.css` and are
> intentionally minimal. This audit targets the **gold-rate** surface users actually price-shop on.
...
**Next step after approval:** Area A plan (tokens + typography) as a short bullet proposal →
implement → before/after screenshots → `npm test`.
