...
export function exportChartCSV(rows, range, karatCode = '24') {
  ...
  const lines = [
    ...
    '',
    csvRow(['Date', 'XAU/USD (spot)', `${karatCode}K USD/gram`, `${karatCode}K AED/gram`, 'Source']),
  ];
  ...
  for (const r of sorted) {
    ...
    const aedGram = usdGram * AED;
    lines.push(csvRow([dateStr, spot.toFixed(2), usdGram.toFixed(3), aedGram.toFixed(3), r.source || 'baseline']));
  }
...
export function exportWatchlistCSV({ favorites, countries, karatCode, priceFor, spot, selectedUnit, selectedCurrency, lang = 'en' }) {
...
export function exportCurrentViewCSV({ countries, karatCode, priceFor, spot, selectedUnit, selectedCurrency, lang = 'en' }) {
  if (!spot) return;
