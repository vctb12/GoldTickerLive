...
function updateShop(id, updates, actor) {
    ...
    const allowedFields = ['name', 'city', 'country', 'type', 'phone', 'email', 
                          'website', 'address', 'latitude', 'longitude', 'hours', 
                          'verified', 'notes'];
...
function batchImportShops(shopList, actor) {
    ...
    shopList.forEach((shopData, index) => {
        try {
            ...
            const newShop = {
                // Use a counter-offset from a single base timestamp to guarantee uniqueness
                id: 'shop_' + (Date.now() + index),
                name: shopData.name.trim(),
                city: shopData.city?.trim() || '',
                country: shopData.country?.trim() || '',
                type: shopData.type || 'direct',
    ...
    saveAllShops(shops);
    logAction(actor, 'create', 'shop', 'batch_import', { count: imported.length, errors: errors.length });
