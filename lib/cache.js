...
export function loadState(STATE) {
  ...
  if (fx) {
    ...
    STATE.status.fxStale = Date.now() > STATE.fxMeta.nextUpdateUtc;
  }

  // Day open
  const dayOpen = safeGet(CACHE_KEYS.dayOpen);
  if (dayOpen && dayOpen.dubaDate === getDubaiDateString()) {
  ...
  if (prefs) {
    ...
    if (prefs.selectedKarat) {
      STATE.selectedKaratSpotlight = prefs.selectedKarat;
      STATE.selectedKaratCountries = prefs.selectedKarat;
    }
...
export function checkDayOpenReset(STATE) {
  ...
  if (!stored || stored.dubaDate !== today) {
    if (STATE.goldPriceUsdPerOz) {
      const payload = { price: STATE.goldPriceUsdPerOz, dubaDate: today };
