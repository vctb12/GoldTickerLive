...
function buildOAuthHeader(method, endpointUrl, ck, cs, at, ts) {
  ...
  const params = { oauth_consumer_key: ck, oauth_nonce: nonce, oauth_signature_method: 'HMAC-SHA1',
    oauth_timestamp: timestamp, oauth_token: at, oauth_version: '1.0' };
  const sortedParams = Object.keys(params).sort()
    .map(k => `${percentEncode(k)}=${percentEncode(params[k])}`).join('&');
  const sigBase = [method.toUpperCase(), percentEncode(endpointUrl), percentEncode(sortedParams)].join('&');
  ...
  return { Authorization: 'OAuth ' + Object.keys(params).sort()
    .map(k => `${percentEncode(k)}="${percentEncode(params[k])}"`).join(', ') };
}
...
async function sendTelegram(text) {
  ...
  await httpsPost(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    chat_id: TELEGRAM_CHANNEL_ID, text, parse_mode: 'HTML', disable_web_page_preview: false,
  });
  ...
  await httpsPost(DISCORD_WEBHOOK, {
    ...
    embeds: [{
      ...
      fields: [
        ...
        { name: 'Change', value: `${sign}$${fmt(changeAbs)} (${sign}${fmt(changePct)}%)`, inline: true },
...
async function sendTweet(spot, changePct, changeAbs, sign) {
  if (!TWITTER_API_KEY || !TWITTER_API_SECRET || !TWITTER_ACCESS_TOKEN || !TWITTER_ACCESS_TOKEN_SECRET) return;
  ...
  const authHeaders = buildOAuthHeader('POST', TWEET_URL,
...
async function main() {
  ...
  console.log(`✅ Spot: $${fmt(spot)}/oz  Open: $${fmt(open)}/oz  Change: ${sign}${fmt(changePct)}%`);

  if (Math.abs(changePct) < THRESHOLD) {
    console.log(`ℹ️  Change (${fmt(changePct)}%) is below threshold (${THRESHOLD}%). No alert needed.`);
    process.exit(0);
  }
