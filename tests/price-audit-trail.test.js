 ...
 * the live pricing layer: spot ÷ 31.1034768 × purity × FX, AED via the 3.6725 peg. Cross-checked against
...
test('audit: 24K AED trail reproduces the raw formula step-by-step', async () => {
  ...
  // Steps: spot → per-gram(÷31.1034768) → purity(×1) → fx(×3.6725)
  assert.deepEqual(
    trail.steps.map((s) => s.key),
    ['spot', 'per-gram', 'purity', 'fx']
  );
  assert.match(trail.steps[1].operation, /÷ 31\.1034768/);
  assert.match(trail.steps[3].operation, /× 3\.6725/);
  assert.match(trail.steps[3].operation, /fixed peg/i);
  // Final equals the raw formula.
  assert.equal(trail.finalPerGram, round4(round4(spot / 31.1034768) * 1 * 3.6725));
});
...
test('audit: final matches the LIVE pricing layer (resolveMetalGramPrice)', async () => {
  ...
  const trail = buildPriceAuditTrail({ spotUsdPerOz: spot, purity: 1, karatCode: '24' });
  const fullPrecisionAed = (spot / 31.1034768) * 3.6725;
  assert.ok(Math.abs(live.aedPerGram - fullPrecisionAed) < 1e-12);
  // The audit trail deliberately chains its four-decimal displayed steps, while the live
  // price keeps full precision until its own display boundary.
  assert.equal(trail.finalPerGram, round4(round4(spot / 31.1034768) * 3.6725));
  assert.ok(Math.abs(trail.finalPerGram - round4(live.aedPerGram)) < 0.00011);
...
test('audit: 22K in USD (fxRate 1) — no peg note, correct purity', async () => {
  ...
  assert.equal(trail.finalPerGram, round4((spot / 31.1034768) * (22 / 24) * 1));
});
...
test('audit: reproducible — multiplying each shown figure reproduces the next output', async () => {
  ...
  const g24 = round4(round2(4149) / 31.1034768);
  ...
  assert.equal(round4(trail.steps[1].input / 31.1034768), trail.steps[1].output);
