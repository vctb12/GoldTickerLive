...
Answer in &lt;3 seconds: _What is gold per gram in AED right now, and is the market open?_
...
`tests/home-hero-loading.test.js`, `tests/home-translations.test.js`,
`tests/first-paint-skeleton.test.js`, `tests/freshness-coverage.test.js`,
...
_Professional workspace_ for live spot, karats, chart, alerts, export — without feeling like 7 apps
in one bag.
...
`tests/tracker-*.test.js` (modes, hero, chart, alerts, export, dom, freshness),
...
`tests/calculator-conversions.test.js`, `tests/calculator-url-sharing.test.js`,
`tests/pricing-formula.spec.test.js`
...
`shops.html`, `src/pages/shops.js`, `src/pages/shops/*`, `data/shops.js`, `styles/pages/shops.css`,
`styles/components/shops-map.css`

### Tests

`tests/shops-compare.test.js`, `tests/verify-shops.test.js`, `tests/shops-business-api.test.js`,
`tests/e2e/shops-search.spec.js`
...
Hash deep links `#compare=ae,sa,kw&k=22`; karat tabs; cheapest callout uses retail formula
documented in tests.
...
Static body without JS (Session 2). `learn.html`, `src/pages/learn.js`, learn-hub renderer if
mounted.

### Insights

BUILD 8 feed: `src/config/insights-articles.js` or `insights-data.js`, `src/pages/insights.js`,
`insights-feed-core.js`.

### Tests

`tests/learn-static-fallback.test.js`, `tests/insights-feed-core.test.js`,
`tests/insights-data.test.js`
...
`countries/**/index.html` (generated), `countries/country-page.js`, `styles/country-page.css`,
generators under `scripts/node/` / `build/`

### Tests

`tests/country-canonical.test.js`, `tests/country-pages-seo.test.js`, `tests/market-intel.test.js`,
`tests/country-consolidation.test.js`
...
**Note:** `GET /api/v1/me/export` and `DELETE /api/v1/me` exist in
`server/routes/public-accounts.js` — verify dashboard wires them; if not, WB-101 is UI-only.
...
1. tracker → 2. index → 3. calculator → 4. shops → 5. methodology → 6. compare → 7. country template
   (pick one CC) → 8. learn → 9. insights → repeat
