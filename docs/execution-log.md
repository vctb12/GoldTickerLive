...
### Implemented
- `utils/routeBuilder.js` — single source of truth for URL generation (buildRoute, buildShopsRoute, buildCanonicalURL, generateAllRoutes)
...
### Implemented
- Security meta headers added to 6 main pages: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`
...
- Created `utils/inputValidation.js` — shared validation utilities (UAE phone, email, numeric range, text sanitization, URL param sanitization, price alert validation)
