...
Replace the homepage `home-chart-section` (single-series USD/oz GoldChart + market-insight panel)
with a dedicated **UAE Historical Gold Rate Chart (AED/g)** showing four karat series with
1M/3M/6M/12M ranges and line/area modes.
...
Revert `index.html`, `home.js`, remove new component/data files, restore prior chart section from
git history. Tracker unaffected.
