...
| Q7  | Flip `FX_INTEGRITY_ENABLED` (src/config/feature-flags.js:37) and wire `sanitizeFxRates` into `fetchFX()` (both live + cache-fallback branches; exact call sites in `docs/plans/midas/FX_MATRIX.md`) — today a truthy corrupt FX rate (e.g. 1e9) flows verbatim into non-AED prices | **Yes** — Phase 8 pinned current behavior with a GAP LOCK test; wiring is a small reviewed change once approved                    |
