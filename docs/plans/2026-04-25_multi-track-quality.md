...
`og:url === canonical` cross-check) ✅ landed. PR 5 (schema/social/nav validation slice:
`inject-schema --check` drift fixed, `check-jsonld-dom.js` added, `og:locale` + `og:image` checks
added, nav A11y/CSS fixes started) 🟡 in progress. **Reconcile into:**
...
- **Validation:** schema.org validator + Google Rich Results Test in CI (additive script).
  _Progress: `scripts/node/check-jsonld-dom.js` now runs in `npm run validate` as the local
  JSON-LD/DOM consistency guard._
- **Constraint:** never schema for content not actually on the page — fail the validator script if a
  JSON-LD field has no DOM equivalent. _Progress: the local guard now checks
  FAQ/WebSite/WebApplication/Article/Product page-content alignment._
...
- **Audit deliverable:** `reports/internal-linking.md`. Build site-wide link graph from generated
  ...
  _Progress: first repo-crawl report added at
...
- **Fix PR:** consolidate to a small `setHead()` helper or per-template constants; ensure canonical
  ...
  for AR/EN parity; ensure `<link rel=alternate hreflang>` pairs are mutual. _Progress: Open Graph
  locale tags added sitewide and `check-seo-meta.js` now enforces `og:locale`,
  `og:locale:alternate`, `og:url === canonical`, and the 1200×630 local `og:image`._
