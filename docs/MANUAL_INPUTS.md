...
Things the repo owner must fill in before the site is fully operational. Each item says what it is,
where to get it, which file to edit, and what breaks without it.
...
- **Where to set it:** Edit `src/config/constants.js`, find the `AD_CONFIG` object near the top. Set
  `ADSENSE_PUBLISHER_ID` to your publisher ID (e.g. `ca-pub-1234567890`) and fill in each key in
  `AD_SLOTS` with the corresponding ad unit ID from your AdSense dashboard.
- **What breaks without it:** Nothing breaks — all ad slots silently render nothing when the
  publisher ID is empty.
...
- **Where to set it:** Edit `src/config/constants.js`, find the `FORMSPREE_ENDPOINT` line near the
  bottom and paste your endpoint URL (e.g. `https://formspree.io/f/xyzabc`).
...
- **Where to get it:** Apply at https://developer.twitter.com for a developer account. Create an app
  with read-write permissions. Get your Consumer Key, Consumer Secret, Access Token, and Access
  Token Secret.
...
- **What breaks without it:** The hourly gold price tweets and spike alerts to X/Twitter will not
  work.
...
- **Where to get it:** Open Telegram, search for @BotFather, send `/newbot`, follow the steps.
  Create a channel, add the bot as admin, get the channel ID.
...
- **Where to get it:** In your Discord server: Server Settings → Integrations → Webhooks → New
  Webhook → Copy URL.
...
- **Where to change it:** If you need a different measurement ID, search-and-replace `G-M14N2GP8ZQ`
...
- **How to do it:** Go to GitHub → Actions tab → find `gold-price-tweet.yml` → click the `…` menu →
  Disable workflow.
...
- **Where to set them:** Search for `AFFILIATE_PLACEHOLDER` in `invest.html` and
  `content/guides/buying-guide.html`. Replace the comment with your actual affiliate link HTML.
...
- **What:** The `node_modules/` directory is currently tracked in the git repository. This adds
  unnecessary size.
...
- **What:** The following JS files in `src/` are not imported by any active code and can be safely
  deleted:
