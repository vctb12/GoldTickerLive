...
| ---------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `src/lib/api.js`                   | ~136    | Fetches gold spot price (XAU/USD) and FX rates with timeout, retry, and simulation hooks. Reads committed JSON; live lane races gold-api.com + mintedmetal.com.             | `fetchGold()`, `fetchFX()`, `setSimulateGoldFail()`, `setSimulateFxFail()`                   |
...
2. lib/api.js → fetchGold()
       │  ├── Try: gold-api.com (live race) or data/gold_price.json
...
| 3   | **Provider chain fragility** | If all providers in `GOLD_PROVIDER_ORDER` fail, prices show fallback/stale labels. Committed JSON + cache mitigate but are not live.          | `gold-price-fetch.yml`, `src/lib/api.js`    |
...
Browser → GitHub Pages (static HTML/JS/CSS)
  └── JS fetches → gold-api.com / committed JSON + open.er-api.com (FX)
...
3. Create API proxy: `/api/gold-price` → serves from provider-adapter chain with server key
