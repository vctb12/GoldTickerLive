...
| 4   | Heatmap RTL tooltip misposition (`heatmap.js:513/526` physical-left → inset-inline-start)                                                                                       | audit A                | ✅ PR #676 (measured: 430px mirror → +13px)                |
| 5   | Portfolio "Add costs to see this" shown for 3 unrelated null-gain causes                                                                                                        | audit D                | ✅ PR #677                                                 |
...
| 15  | Tracker a11y touch-ups: SVG tooltip mouse-only → pointermove; skip-link `#main-content` vs tracker `<main id="tracker-app">`                                                    | audits A+E             | ✅ PR #678 (skip-link half re-verified NOT-A-BUG, dropped) |
...
| 18  | `.edu-table-wrap` scrollable regions not keyboard-focusable (axe serious ×20 on compare/portfolio/heatmap) — add tabindex=0 + role/label to the shared pattern                  | audit E (high)         | ✅ PR #679                                                 |
...
| 21  | Wire existing deterministic `buildMarketAnalysis()` (tested, bilingual, LLM-free) into market page as "Today's movement, described" panel — zero new infra                      | audit G (high)         | ✅ PR #680 (incl. adverb→adjective grammar fix)            |
