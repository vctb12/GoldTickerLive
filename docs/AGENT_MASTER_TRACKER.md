...
no term content or visible copy changed; static `BreadcrumbList` JSON-LD preserved.
...
| 2026-07-09 | Claude Code | Program G / G16 (branch `claude/g16-glossary-definedtermset-2ksva9`, additive minimal-diff): glossary `DefinedTermSet` structured data. New `src/seo/glossary-schema.js` (`collectGlossaryTerms` reads active-locale `[data-lang-block]` DOM, `buildGlossaryTermSetSchema` emits `DefinedTermSet` + one `DefinedTerm` per term, `injectGlossarySchema`/`syncGlossarySchema` inject idempotently by id); wired into `src/pages/glossary.js` `applyLang()` so schema regenerates for the ACTIVE locale on load and on language toggle. EN emits bare-URL term anchors; AR emits `?lang=ar` set URL and no leaked English. Static `BreadcrumbList` untouched; no visible copy changed. +12 tests (`tests/glossary-schema.test.js`). `npm run lint` / `node --test` (1419 pass) / `npm run build` / `npm run validate` all green. |
