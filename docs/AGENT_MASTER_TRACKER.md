...
**Last updated:** 2026-07-09 · **Updated by:** Claude Code **Purpose:** The single canonical source
...
| Overnight agent automation — safe tooling & guardrails   | `claude/overnight-agent-automation-i6l4dc`    | in-progress | —   | SAFE runner + status script + .claude deny/allow + additive agent-ci + Codex review prompt + handoffs; no merge/deploy/secrets, peg/formulas untouched |
...
| 2026-07-09 | Claude Code | Overnight agent automation (branch `claude/overnight-agent-automation-i6l4dc`, tooling + docs only, additive minimal-diff): added SAFE overnight runner `scripts/agent-night-run.sh` (no permission bypass, never `main`, logs to `logs/agent-night/`), read-only `scripts/agent-status.sh`, `.claude/settings.json` allow/deny policy (denies push/force-push to main, merge, deploy, `.env`/secrets, `gold-price-fetch.yml`/`post_gold.yml`/`sw.js`, `src/config/constants.js`), additive `.github/workflows/agent-ci.yml` (agent branches only; no deploy/secrets/schedule; existing `ci.yml` untouched), `.github/codex/prompts/review.md` reviewer prompt, `docs/agent-handoffs/` + README, AGENTS.md/CLAUDE.md guardrail sections, `.gitignore` log-dir entry. No merge/deploy/secrets; `gold-price-fetch.yml`/`post_gold.yml`/`sw.js` and AED 3.6725 peg / troy 31.1034768 g / price-karat-FX formulas / source-priority logic all untouched. |
