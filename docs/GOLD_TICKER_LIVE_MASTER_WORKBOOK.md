...
> **This is the one document.** Everything else is reference, history, or a thin index pointing
> here. **v2 adds deep appendices** — per-surface playbooks, API map, CI catalog, step-by-step
> session guides, test map, owner secrets, country/SEO strategy.
>
...
>
> **Session log:**
> [`docs/workbook/WORKBOOK_SESSION_REGISTRY.md`](./workbook/WORKBOOK_SESSION_REGISTRY.md)
...
- [`docs/plans/2026-06-01_master-operations-hub.md`](./plans/2026-06-01_master-operations-hub.md) —
  short routing summary → **prefer this workbook**
...
2. Open [`PLAN.md`](../PLAN.md) and
   [`WORKBOOK_SESSION_REGISTRY.md`](./workbook/WORKBOOK_SESSION_REGISTRY.md).
3. Pick **one** session from [Part 6](#part-6--session-catalog--complete-work-queue) (or run
   [Part 8](#part-8--discovery-scanners) to mint a micro-session).
...
**Your job now:** close **integration gaps**, **product focus** (page sprawl), **vendor/GDPR**, and
**terminal-grade UX** on flagship surfaces.
...
2. **Freshness** — Every visible price has source, timestamp, and state (`live` / `cached` /
   `fallback` / …).
...
5. **Depth** — Methodology, learn, insights, calculator, compare — all subordinate to **live price
   truth**.
...
**Deep dive:** [`docs/freshness-contract.md`](./freshness-contract.md),
[`.github/instructions/gold-pricing.instructions.md`](../.github/instructions/gold-pricing.instructions.md)
...
**Execution detail for starred sessions:**
[Appendix D](./workbook/APPENDIX_D_SESSION_EXECUTION_GUIDES.md)
...
Single table — source rows from `PLAN.md`, audits, BUILD catalog. **Do not duplicate into new plan
files.**
...
Also read the matching guide in docs/workbook/APPENDIX_D_SESSION_EXECUTION_GUIDES.md when executing
a numbered WB session.
```
...
**NEXT_PR_SEQUENCE** maps PR1–9 to remaining glue: see
