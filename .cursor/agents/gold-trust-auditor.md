...
description:
  Gold pricing and freshness trust specialist for Gold Ticker Live. Use proactively when editing
  visible prices, country pages, calculator output, freshness labels, or karat factors. Never
  weakens reference-vs-retail distinction.
---
...
2. Read src/config/constants.js and src/config/karats.js (do not change peg 3.6725 or troy ounce
...
Return: findings by severity (critical | warning | nit) with file:line evidence. Fix only if the
user asked you to implement; otherwise report only.
