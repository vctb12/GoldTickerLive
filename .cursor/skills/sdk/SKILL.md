...
name: sdk
description:
  Guide users building apps, scripts, CI pipelines, or automations on top of the Cursor SDK -
  TypeScript (@cursor/sdk) or Python (cursor-sdk / cursor_sdk). Use when integrating Cursor agents
  programmatically; Agent.create, Agent.prompt, streaming, local vs cloud runtime, MCP, errors.
---
...
- **TypeScript** (`@cursor/sdk`, npm) —
  [cursor.com/docs/sdk/typescript](https://cursor.com/docs/sdk/typescript)
...
3. Else ask: _"TypeScript or Python?"_
...
1. **Wrong runtime** — explicitly set `local: { cwd }` or `cloud: { repos: [...] }`; default is
   local silently.
2. **Two failure kinds** — thrown `CursorAgentError` = never started; `result.status === "error"` =
   ran and failed.
