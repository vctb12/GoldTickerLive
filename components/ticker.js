...
const TICKER_ITEMS = [
  { key: 'xauUsd', labelEn: 'XAU/USD',         labelAr: 'XAU/USD',               fmt: v => `$${v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}` },
  { key: 'uae24k', labelEn: 'UAE 24K/g (AED)',  labelAr: 'الإمارات 24K/غ (AED)',  fmt: v => v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}) },
  { key: 'uae22k', labelEn: 'UAE 22K/g (AED)',  labelAr: 'الإمارات 22K/غ (AED)',  fmt: v => v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}) },
  { key: 'uae21k', labelEn: 'UAE 21K/g (AED)',  labelAr: 'الإمارات 21K/غ (AED)',  fmt: v => v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}) },
  { key: 'uae18k', labelEn: 'UAE 18K/g (AED)',  labelAr: 'الإمارات 18K/غ (AED)',  fmt: v => v.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}) },
];
...
function buildCopyHtml(lang, trackerHref) {
  ...
  const items = TICKER_ITEMS.map(item => {
    ...
    return `<a href="${trackerHref}" class="ticker-item" data-key="${item.key}" tabindex="-1">` +
...
export function injectTicker(lang = 'en', depth = 0) {
  ...
  document.getElementById('ticker-close')?.addEventListener('click', () => {
    ticker.classList.add('ticker-dismissed');
    document.body.classList.remove('has-ticker');
    try { sessionStorage.setItem(DISMISSED_KEY, '1'); } catch { /* blocked */ }
  });
...
export function updateTicker(data = {}) {
  TICKER_ITEMS.forEach(item => {
    ...
    document.querySelectorAll(`.ticker-item[data-key="${item.key}"] .ticker-value`).forEach(el => {
...
export function updateTickerLang(lang) {
  ...
  TICKER_ITEMS.forEach(item => {
    const label = isAr ? item.labelAr : item.labelEn;
    document.querySelectorAll(`.ticker-item[data-key="${item.key}"] .ticker-label`).forEach(el => {
